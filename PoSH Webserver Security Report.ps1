...
$DateFDT = (Get-Date -Format FileDateTime)
...
$validatedurls = "$URL/wordpress/readme.html", "$URL/wordpress/readme.html", "$url/xmlrpc.php", "$url/wordpress/xmlrpc.php", "$url/global.asa" , "$url/admin/createUser.php?member=myAdmin", "$url/admin/changePw.php?member=myAdmin&passwd=foo123&confirm=foo123", "$url/admin/groupEdit.php?group=Admins&member=myAdmin&action=add", "$url/admin/createUser.php?member=myAdmin", "$url/phpinfo.php" | Validate-Url | ConvertTo-Html
...
<TR>
<TD>
<CENTER><B><FONT SIZE="16">PowerShell Webserver Security Report for <FONT COLOR=RED>$URL</FONT></FONT></B><CENTER/>
...
# Export to HTML
$Script | ConvertTo-HTML -Head $HTMLHead -Body $HTMLBody | Out-file "C:\temp\PoSh-Security-Report-$URL-$DateFDT.html"
